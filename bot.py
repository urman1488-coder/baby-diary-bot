...
WEBHOOK_HOST = 'https://baby-diary-bot-1.onrender.com'import logging
from datetime import datetime, timedelta
import pytz
import asyncio
from aiogram import Bot, Dispatcher, types, F
...
# Обработчик нажатия на inline-кнопку (Вариант 1B)
...
async def handle_wakeup_callback(callback: types.CallbackQuery):
    ...
    # Обновляем сообщение с результатом (Вариант 1B)
...
if __name__ == '__main__':
    ...
    web.run_app(app, host='0.0.0.0', port=port)TOKEN}'
WEBHOOK_URL = f"{WEBHOOK_HOST}{WEBHOOK_PATH}"
