...
# Функция для удаления дублирующих сообщений бота (60 секунд)
async def delete_bot_duplicates(chat_id: int, new_text: str, new_message_id: int):
    ...
    Удаляет ТОЛЬКО если:
    1. Сообщение отправлено в последние 60 секунд
    ...
    # Ищем похожие сообщения за последние 60 секунд
    for msg in chat_messages:
        time_diff = (current_time - msg["time"]).seconds
        text_match = msg["text"] == new_text
        
        # Если нашли похожее сообщение за последние 60 секунд
        if time_diff < 60 and text_match:  # Изменено с 5 на 60 секунд
            logger.info(f"  🚨 НАЙДЕН ДУБЛЬ! Разница {time_diff} сек < 60 сек")
            ...
            if time_diff >= 60:
                logger.info(f"  ⏭️ Сообщение старше 60 сек ({time_diff} сек) - не дубль")
            if not text_match:
